	...
	<script language="Javascript">
		...
		function valida(){
			...
			if(document.f1.nome.value==""){
				alert("A Coleção ...... tem que ser preenchida");
				vazio=true
			}
			...
			if(document.f1.city.value==""){
				alert("O Ano ...... tem que ser preenchido");
				vazio=true
			}
			if(document.f1.cod.value==""){
				alert("A Edição ...... tem que ser preenchida");
				vazio=true
			}
			if(document.f1.loc.value==""){
				alert("A Localização ...... tem que ser preenchida");
				vazio=true
			}
...
<ul id = "NavMenu">
	...
	$sql = "SELECT * FROM revista where nome like '%$pesquisa%'";
	...
	<div id = "G1">
			...
			<form name="f1" method = "POST" action="altrev2.php">
				<fieldset>		
					<p class = "TituloForm2">
						Alterar Amigo
					</p>
					...
					<p class = "TextoForm">
						...
						<input type = "text" name = "nome" id = "nome" value='<?php echo $row['colecao'] ?>'/>
					</p>
					<p class = "TextoForm">
						<label> Nome: </label><br>
						<input type = "text" name = "end" id = "end" value='<?php echo $row['nome'] ?>'/>
					</p>
					<p class = "TextoForm">
						<label> Ano: </label><br>
						<input type = "text" name = "city" id = "city" value='<?php echo $row['ano'] ?>'/>
					</p>
					<p class = "TextoForm">
						<label> Edição: </label><br>
						<input type = "text" name = "cod" id = "cod" value='<?php echo $row['edicao'] ?>'/>
					</p>
					<p class = "TextoForm">
						<label> Localização: </label><br>
						<input type = "text" name = "loc" id = "loc" value='<?php echo $row['localizacao'] ?>'/>
					</p>
					<br><br>
